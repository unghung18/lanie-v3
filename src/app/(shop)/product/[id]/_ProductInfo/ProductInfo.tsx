...
const ProductInfo = ({ product }: { product: ProductProps }) => {
  ...
  const openQuickView = useAppSelector(
    (state) => state.toggleQuickview.toggleQuickview
  );
    ...
    <div className="product-info">
      <div className="product-info__header">
        <div style={{ display: "flex", flexDirection: "column" }}>
          ...
          <h1 className="product-info__header--name">{product?.title}</h1>
        </div>
        <div
          className={`product-info__header-wishlist ${
            isExits() ? "active" : ""
          }`}
      ...
      <div className="product-info__color">
        <div className="product-info__color-heading">
          <span>Màu sắc: </span>
          <span>
            <strong>{selectedColor && selectedColor} </strong>
          </span>
        </div>
      ...
      <div className="product-info__size">
        <div className="product-info__size-heading">
          ...
          {selectedSize == "s" && (
            <div>
              Kích cỡ:<strong> {selectedSize.toUpperCase()}</strong>
              <span>(1m50 - 1m60 | 45kg - 50kg)</span>
            </div>
          )}
          {selectedSize == "m" && (
            <div>
              Kích cỡ:<strong> {selectedSize.toUpperCase()}</strong>
              <span>(1m55 - 1m60 | 51kg - 55kg)</span>
            </div>
          )}
          {selectedSize == "l" && (
            <div>
              Kích cỡ:<strong> {selectedSize.toUpperCase()}</strong>
              <span>(1m55 - 1m60 | 56kg - 60kg)</span>
            </div>
          )}
          {selectedSize == "xl" && (
            <div>
              Kích cỡ:<strong> {selectedSize.toUpperCase()}</strong>
              <span>(1m60 - 1m65 | 61kg - 64kg)</span>
            </div>
          )}
          {selectedSize == "2xl" && (
            <div>
              Kích cỡ:<strong> {selectedSize.toUpperCase()}</strong>
              <span>(1m60 - 1m65 | 65kg - 68kg)</span>
            </div>
          )}
          {selectedSize == "3xl" && (
            <div>
              Kích cỡ:<strong> {selectedSize.toUpperCase()}</strong>
              <span>(1m60 - 1m65 | 69kg - 70kg)</span>
            </div>
          )}
          <span
            style={{
        ...
        <ul className="product-info__size-list">
          {product?.sizes.map((size, index) => (
            <li
              ...
              className={`${
                selectedSize && selectedSize == size.name && "active"
              }`}
            >
        ...
        <div className="product-info__quantity-button">
          ...
          {selectedColor ? (
            selectedSize ? (
              ...
              <div
                className="button"
                style={{ gap: "4px", opacity: "0.7", pointerEvents: "none" }}
              >
                <MdOutlineShoppingBag />
      ...
      >
        ...
        <span style={{ fontSize: "14px", fontWeight: "700", color: "#2F5ACF" }}>
          Chat để được Lanie tư vấn ngay (8:30 - 22:00)
        </span>
      </Link>
        ...
        <ul>
          <li style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            <span style={{ color: "#2e2e2e", fontSize: "14px" }}>
              Miễn phí giao hàng toàn quốc cho đơn hàng trên 200,000đ.
            </span>
          </li>
          <li style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <div
            ...
            ></div>
            <span style={{ color: "#2e2e2e", fontSize: "14px" }}>
              Hỗ trợ tư vấn từ 8:30 - 22:00 mỗi ngày
            </span>
          </li>
          <li style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <div
