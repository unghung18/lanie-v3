...
const Page = ({ params }: {
    ...
    const onChangeDelivaryMethod = (event: React.ChangeEvent<HTMLInputElement>) => {
        setDeliveryMethod(event.target.value);
    }
    const onChangePaymentMethod = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        <>
            <div className='px-5 md:px-10 lg:px-40 max-w-[1280px] mx-auto grid md:grid-cols-10 gap-5 py-10 bg-gradient-to-r from-[#faf8f1] to-[#f6f3ef] grid-cols-1 min-h-[100vh]'>
                <div className=' space-y-3 col-span-7 bg-white py-4 px-5'>
                    ...
                    <input onChange={handleChangeInput} name='name' type="text" placeholder='Họ và tên' className='w-full placeholder:font-extralight placeholder:text-gray-500 outline-none border-[1px] p-2 rounded-md border-gray-400 text-[#333333] shadow-sm focus:border-[#338dbc] focus:border-[1px]' />
                    <div className='grid grid-cols-10'>
                        <input onChange={handleChangeInput} name='email' type="email" placeholder='Email' className='col-span-7 placeholder:font-extralight placeholder:text-gray-500 outline-none border-[1px] p-2 rounded-md border-gray-400 mr-3 text-[#333333] shadow-sm focus:border-[#338dbc] focus:border-[1px]' />
                        <input onChange={handleChangeInput} name='phone' type="text" placeholder='Điện thoại' className='col-span-3 placeholder:font-extralight placeholder:text-gray-500 outline-none border-[1px] p-2 rounded-md border-gray-400 text-[#333333] shadow-sm focus:border-[#338dbc] focus:border-[1px]' />
                    </div>
                    <input onChange={handleChangeInput} name='address' type="text" placeholder='Địa chỉ số nhà, đường, ngõ,...' className='w-full placeholder:font-extralight placeholder:text-gray-500 outline-none border-[1px] p-2 rounded-md border-gray-400 text-[#333333] shadow-sm focus:border-[#338dbc] focus:border-[1px]' />
                    <div className='grid grid-cols-3 gap-5'>
                        ...
                        <div className='p-2 border rounded-md border-gray-400 flex items-center'>
                            <div className='flex flex-col w-full'>
                                ...
                                <select id="district" className='outline-none' onChange={handleChangeDistrict}>
                                    <option value="">Chọn quận huyện</option>
                                    {districts && districts.map((e: any) => (
                                        <option value={e.Id} key={e.Id}>{e.Name}</option>
                                    ))}
                                </select>
                            </div>
                        ...
                        <div className='p-2 border rounded-md border-gray-400 flex items-center'>
                            <div className='flex flex-col w-full'>
                                ...
                                <select id="ward" className='outline-none' onChange={handleChangeWard}>
                                    <option value="default">Chọn phường xã</option>
                                    {wards && wards.map((e: any) => (
